...
class PVModel:
    ...
    def run_model(self, real: pd.DataFrame = None, sample: list[pd.DataFrame] = None, years: list[int] = [2022],
                  month: int = 1, resolution: int = 60):
        complete_sample = []
        if real is None:
            # Choosing random date
            data_to_refer = {
                'year': np.random.choice(years),
                'month': month
            }
            # Generating random day sequence (needed by pvlib)
            date_complete = pd.date_range(start=f"{data_to_refer['year']}-{data_to_refer['month']:02d}-01",
                                          periods=len(sample))
            # date_complete += pd.DateOffset(days=np.random.randint(0, 31))

            for df, date in zip(sample, date_complete):
                # Convertire gli orari in timestamp completi
                if real is None:
                    df.index = pd.date_range(start=date, periods=24 * 60 / resolution, freq=f'{resolution}min')
