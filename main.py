...
if generate == 'n' and path.exists("synth_results.npy"):
    ...
    # Saving samples
    np.save("synth_results.npy", synth_results)
...
def verifica_pattern(finestra):
    ...
    if ore_notturne_pre.max() <= soglia_bassa and ore_notturne_post.max() <= soglia_bassa:
        return True
    return False
...
for df, date in zip(finestre_valide, date_complete):
